...
export function UploadZone({ onUpload }: UploadZoneProps) {
  ...
  return (
    <Card className="h-full border-border bg-card rounded-none relative overflow-hidden group flex flex-col">
      ...
      <CardContent className="flex-1 min-h-0 flex flex-col gap-4">
        {stagedFiles.length === 0 ? (
          <div 
            className={`
              relative border-2 border-dashed transition-all duration-300 flex-1 flex flex-col items-center justify-center min-h-[180px]
          ...
          >
            ...
            <div className="flex flex-col items-center gap-4 z-10">
              <div className={`p-4 border border-border bg-background transition-transform duration-300 ${dragActive ? "scale-110" : ""}`}>
              ...
              <Button 
                ...
                className="rounded-none border-primary text-primary hover:bg-primary hover:text-primary-foreground font-mono text-xs uppercase tracking-wider"
              >
                Select Files
              </Button>
            </div>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <ScrollArea className="flex-1 border border-border bg-background/50">
              <div className="p-2 space-y-2">
                {stagedFiles.map((file, i) => (
                  <div key={i} className="flex items-center justify-between p-2 bg-card border border-border group">
                    ...
                    <Button 
                      variant="ghost" 
                      size="icon" 
                      className="h-6 w-6 rounded-none hover:text-destructive"
            ...
            <div className="pt-4 flex gap-2">
              <Button 
                variant="outline" 
                className="flex-1 rounded-none border-muted-foreground/30"
