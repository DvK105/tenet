...
function validateBlendFile(arrayBuffer) {
    try {
        ...
        if (fileVersionNum > 405) {
            console.warn(`[render-job] WARNING: Blender file version (${blenderFileVersion}) may be newer than sandbox Blender (4.5.0). This may cause errors.`);
        }
        return { valid: true };
    }
    catch (err) {
...
export const renderJob = inngest.createFunction({ id: "render-job" }, { event: "render/uploaded" }, async ({ event, step }) => {
    ...
    const { sandboxId, tmpDir, frameStart, frameEnd, framesPerBatch } = await step.run("download-and-setup", async () => {
        ...
        if (downloadError || !data) {
            const errorMessage = `Failed to download input file from Supabase (bucket=${inputBucket}, path=${id}): ${downloadError?.message ?? "no data returned"}`;
        ...
        try {
            ...
            if (fileSize !== blendArrayBuffer.byteLength) {
                console.warn(`[render-job] WARNING: File size mismatch. Expected: ${blendArrayBuffer.byteLength}, Got: ${fileSize}`);
            }
        }
        catch (err) {
            await sandbox.kill().catch(() => { });
...
`;
        ...
        let frameRangeResult;
        try {
            frameRangeResult = await sandbox.commands.run(`${BLENDER_BIN} -b "${sceneFilePath}" -P "${getFrameRangeScript}" --background`, { timeoutMs: 300000 });
        }
        catch (err) {
            await sandbox.kill().catch(() => { });
            ...
            throw new Error(`Failed to get frame range from Blender: ${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        if (frameRangeResult.stderr) {
            ...
            if (stderrLower.includes("error") ||
                ...
                throw new Error(`Blender cannot open the file. This usually means the file was created with a newer Blender version than 4.5.0. Error: ${frameRangeResult.stderr}`);
            }
        }
        // Parse frame range
        let frameStart = 1;
        ...
        try {
            ...
            if (frameRangeMatch) {
                ...
                console.warn(`[render-job] Could not parse frame range from output: ${frameRangeResult.stdout}. Using defaults.`);
            }
        }
        catch (err) {
    ...
    console.log(`[render-job] Step 2+: Starting batch rendering. Total frames: ${frameEnd - frameStart + 1}, Batch size: ${framesPerBatch}`);
    ...
    while (currentFrame < frameEnd) {
        ...
        await step.run(`render-batch-${stepCount}`, async () => {
            ...
            try {
                ...
                throw new Error(`Failed to reconnect to sandbox ${sandboxId} for batch ${stepCount}. Sandbox may have timed out. Error: ${err instanceof Error ? err.message : String(err)}`);
            }
            try {
                const sceneFilePath = `${tmpDir}/scene.blend`;
                const framesDir = `${tmpDir}/frames`;
...
`;
                ...
                try {
                    ...
                    throw new Error(`Blender render failed for batch ${stepCount} (frames ${batchStart}-${nextBatchEnd}): ${err instanceof Error ? err.message : String(err)}. Exit code: ${err?.exitCode || "unknown"}. Stderr: ${err?.stderr || "none"}`);
                }
                ...
                try {
                    ...
                    if (frameCount === 0 && stepCount === 0) {
                        ...
                        throw new Error(`No frames were rendered in the first batch. Check Blender output for errors. Stdout: ${renderResult.stdout}, Stderr: ${renderResult.stderr}`);
                    }
                }
                catch (err) {
    ...
    const outputVideoPath = await step.run("encode-and-upload", async () => {
        ...
        try {
            ...
            throw new Error(`Failed to reconnect to sandbox ${sandboxId} for encoding. Sandbox may have timed out. Error: ${err instanceof Error ? err.message : String(err)}`);
        }
        try {
            const framesDir = `${tmpDir}/frames`;
            ...
            try {
                ...
                throw new Error(`FFmpeg encoding failed: ${err instanceof Error ? err.message : String(err)}. Exit code: ${err?.exitCode || "unknown"}. Stderr: ${err?.stderr || "none"}`);
            }
            ...
            try {
                ...
                if (!verifyVideo.stdout.includes("exists")) {
                    throw new Error(`FFmpeg completed but output video file was not created. FFmpeg stdout: ${ffmpegResult.stdout}, stderr: ${ffmpegResult.stderr}`);
                }
            }
            catch (err) {
            ...
            let fileData;
            try {
                fileData = (await sandbox.files.read(outputVideoPath));
            }
            catch (err) {
            ...
            if (uploadResult.error) {
                ...
                throw new Error(`Supabase upload failed (bucket=${outputBucket}, path=${uploadPath}): ${uploadResult.error.message}`);
            }
